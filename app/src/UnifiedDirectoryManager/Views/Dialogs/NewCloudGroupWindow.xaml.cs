...
public partial class NewCloudGroupWindow : Window
{
    public NewCloudGroupWindow()
    {
        InitializeComponent();
        this.FixLazyRender();
    }
    ...
    protected override void OnClosing(CancelEventArgs e)
    {
        ...
        // orphan a half-created group and then set DialogResult on a closed window. The success path sets
        // DialogResult=true before Close(), so it's allowed through.
        if (DialogResult != true && DataContext is NewCloudGroupViewModel { IsBusy: true })
            e.Cancel = true;
        base.OnClosing(e);
    }
}
