...
public partial class NewUserViewModel : ObservableObject
{
    ...
    /// <summary>
    /// Recomputes suggested email / UPN / proxy values from the template and the entered name.
    /// When not forcing, only fields the user hasn't manually changed are updated.
    /// </summary>
    private void ApplySuggestions(bool force)
    {
        ...
        var emailSuggest = SelectedTemplate.AttributeDefaults.TryGetValue("mail", out var mailPat)
            ? Resolve(mailPat, sam) : string.Empty;
        var upnSuggest = SelectedTemplate.AttributeDefaults.TryGetValue("userPrincipalName", out var upnPat)
            ? Resolve(upnPat, sam)
            : (!string.IsNullOrWhiteSpace(UpnSuffix) && !string.IsNullOrWhiteSpace(sam) ? $"{sam}@{UpnSuffix.Trim()}" : string.Empty);
        var proxySuggest = string.Join(Environment.NewLine,
            SelectedTemplate.ProxyAddressPatterns.Select(p => Resolve(p, sam)).Where(s => !string.IsNullOrWhiteSpace(s)));

        if (force || Email == _lastEmailSuggestion) { Email = emailSuggest; _lastEmailSuggestion = emailSuggest; }
        if (force || Upn == _lastUpnSuggestion) { Upn = upnSuggest; _lastUpnSuggestion = upnSuggest; }
        if (force || ProxyAddressesText == _lastProxySuggestion) { ProxyAddressesText = proxySuggest; _lastProxySuggestion = proxySuggest; }
    }

    private IReadOnlyList<string> ResolvedProxies() =>
        ProxyAddressesText.Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries).ToList();
    ...
    /// <summary>Resolves the template defaults + computed cn/sam/upn into a concrete attribute set.</summary>
    private Dictionary<string, string> BuildAttributes()
    {
        var result = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);
        if (SelectedTemplate is null) return result;

        var sam = ComputeSam();

        foreach (var (ldap, pattern) in SelectedTemplate.AttributeDefaults)
        {
            if (ldap.Equals("sAMAccountName", StringComparison.OrdinalIgnoreCase)) continue;
            var value = Resolve(pattern, sam);
            if (!string.IsNullOrWhiteSpace(value)) result[ldap] = value;
        }

        // Ensure the essentials exist.
        result["sAMAccountName"] = sam;
        if (!result.ContainsKey("givenName") && !string.IsNullOrWhiteSpace(FirstName)) result["givenName"] = FirstName.Trim();
        if (!result.ContainsKey("sn") && !string.IsNullOrWhiteSpace(LastName)) result["sn"] = LastName.Trim();
        if (!result.ContainsKey("middleName") && !string.IsNullOrWhiteSpace(MiddleName)) result["middleName"] = MiddleName.Trim();
        if (!result.ContainsKey("displayName")) result["displayName"] = $"{FirstName} {LastName}".Trim();
        if (!result.ContainsKey("cn")) result["cn"] = result.TryGetValue("displayName", out var dn) && !string.IsNullOrWhiteSpace(dn) ? dn : sam;
        if (!result.ContainsKey("userPrincipalName") && !string.IsNullOrWhiteSpace(UpnSuffix)) result["userPrincipalName"] = $"{sam}@{UpnSuffix.Trim()}";
        ...
        // Manager (DN); the picker is authoritative — overrides any template default, or clears it.
    ...
    private string ComputeSam()
    {
        if (SelectedTemplate is null) return string.Empty;
        var samPattern = SamOverride;
        if (string.IsNullOrWhiteSpace(samPattern))
            SelectedTemplate.AttributeDefaults.TryGetValue("sAMAccountName", out samPattern);
        if (string.IsNullOrWhiteSpace(samPattern)) samPattern = "{first}.{last}";
        return Sanitize(Resolve(samPattern, sam: string.Empty));
    }
    ...
    private string Resolve(string pattern, string sam)
    {
        ...
        return Regex.Replace(pattern,
            ...
            m => m.Groups[1].Value.ToLowerInvariant() switch
            {
                ...
                "middleinitial" => Initial(MiddleName),
